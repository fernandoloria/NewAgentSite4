...
function GetDetail(Send) {
    ...
    $.get('/Popup/ChartStraightValue.aspx?Data=' + Send, function (data) {
        var content = $(data);
        var divContent = content.find('#dgsContent').html();
...
function GetPlayerHistory(Id, Date, Day) {
    ...
    Date = Date + "";
    Day = Day + "";
    console.log('/Popup/WeeklyBalanceDayHistory.aspx?Data=' + Id + '_' + Date + '_' + Day + '_' + sCurrency);
...
document.getElementById('searchInput')?.addEventListener('input', function (e) {
    ...
    debounceTimeout = setTimeout(() => {
        ...
        const filteredUsers = users.filter(user => user.Account.toLowerCase().includes(searchTerm));
...
function displayUsers(userArray) {
    ...
    const userlimitedArray = userArray.slice(0, 50);

    userlimitedArray.forEach(user => {
        const li = document.createElement('li');

        const typeSpan = document.createElement('span');
        typeSpan.innerText = user.Type.toUpperCase();
        switch (user.Type) {
            case 'P':
                typeSpan.className = 'type-p';
                break;
            case 'M':
                typeSpan.className = 'type-m';
                break;
            case 'A':
                typeSpan.className = 'type-a';
                break;
            case 'D':
                li.className = 'type-d';
                break;
        ...
        if (user.Type == "D") {
            li.appendChild(document.createTextNode(`${user.Account}`));
        } else {
            li.appendChild(document.createTextNode(`${user.Account} (${user.password}) ${user.Name}`));
            li.appendChild(spaceSpan);
        ...
        typeSpan.addEventListener('click', function (event) {
...
function updateSession(id) {
    $('.preloader').show();
    fetch('/services/ChangeSubAgent.ashx', {
        method: 'POST',
...
document.addEventListener('click', function (event) {
    ...
    if (event.target !== searchInput && event.target !== userList && !userList.contains(event.target)) {
...
//function GetExposureDetail(idGame, WT, P) {
//    var Currency = document.getElementById('cCurrency');
//    $.get('/PopUp/PopUpAgentExposureDetail.aspx?idGame=' + idGame + '&WT=' + WT + '&p=' + P, function (data) {
//        var content = $(data);
//        var divContent = content.find('#dgsContent').html();
//        $('#PopUpModal .modal-body').html(divContent);
//        $('#PopUpModal').modal('show');
//    });
//    //    window.open('/Popup/PopUpAgentExposureDetail.aspx?idGame=' + idGame + '&WT=' + WT + '&p=' + P, null, 'scrollbars=yes,directories=no,height=400,width=700,menubar=no,left=50,top=80');
//}

function cancelWagerPopup(Id) {
    Id = Id + "";
    window.open('/Popup/CancelWagerPopUp.aspx?tn=' + Id + '', null, 'scrollbars=yes,directories=no,height=400,width=700,menubar=no,left=50,top=80');
}

function cancelWagerPopup(Id, IdSport) {
    Id = Id + "";
    window.open('/Popup/CancelWagerPopUp.aspx?tn=' + Id + '&idsport=' + IdSport + '', null, 'scrollbars=yes,directories=no,height=400,width=700,menubar=no,left=50,top=80');
}
