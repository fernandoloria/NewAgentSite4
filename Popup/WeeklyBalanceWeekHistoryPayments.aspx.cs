...
{
    public partial class WeeklyBalanceWeekHistoryPayments : System.Web.UI.Page
    {
        ...
        private void LoadData()
        {
            ...
            if (this.Request.QueryString["Data"] != null)
            {
                ...
                prmEndDate = dateTime.AddDays(double.Parse("7") - 1.0).ToString("MM/dd/yyyy");
        ...
        private CResultPlayerHistory GetReportPlayerHistory(long prmIdAgent, long prmIdPlayer, string prmStartDate, string prmEndDate, int prmHistWeek, int prmPage, int prmRecPecPage, string prmNextQ, short prmIdCurrency, bool prmNormalOrder)
        {
            ...
            try
            {
                ...
                while (index1 < playerHistoryByPage.Tables[0].Rows.Count)
                {
                    ...
                    string TRisk;
                    string TWin;
                    if (prmStartDate == prmEndDate)
                    {
                        lblHeader.Text = "PLAYER: " + Player + "<br/> HISTORY: " + Convert.ToDateTime(prmStartDate).ToString("MM/dd");
                    }
                    else
                    {
                        lblHeader.Text = "PLAYER: " + Player + "<br/> HISTORY: " + Convert.ToDateTime(prmStartDate).ToString("MM/dd") + " - " + Convert.ToDateTime(prmEndDate).ToString("MM/dd");
                    }
        ...
        public DataSet GetPlayerHistoryByPage(long prmIdAgent, long prmIdPlayer, string prmStartDate, string prmEndDate, int prmPage, int prmRecsPerPage, bool prmNormalOrder)
        {
            ...
            try
            {
                ...
                ((SqlParameter)comm.Parameters.Add("@prmTranType", SqlDbType.Int)).Value = 2;
