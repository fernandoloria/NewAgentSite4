...
{
    public partial class PlayerAccess : BasePage, IRequiresSessionState
    {
            ...
            try
            {
               ...
               AddOnWebClient.Agent agentInstance = new AddOnWebClient.Agent();
                if (Common.HasRights(ReportPosition.PLAYERACCESS))
                {
                    ...
                    string prmStartDate = this.Request.Form["Date1"] == null ? DateTime.Today.ToString("MM/dd/yyyy") : this.Request.Form["Date1"].ToString();
                    string prmEndDate = this.Request.Form["Date2"] == null ? DateTime.Today.AddDays(1.0).ToString("MM/dd/yyyy") : this.Request.Form["Date2"].ToString();
                    if (this.Request.Form["cPlayer"] != null)
                        prmIdPlayer = int.Parse(this.Request.Form["cPlayer"].ToString());
                    if (this.Request.Form["txtIP"] != null)
                        prmIP = this.Request.Form["txtIP"].ToString();
                    CResultAgentPlayerAccess playerAccessInfo = agentInstance.GetReportAgentPlayerAccessInfo(prmIdAgent, prmIdPlayer, prmStartDate, prmEndDate, prmIP);
                    if (playerAccessInfo.ErrorCode == CErrorCode.ErrorNone)
                    {
                        string str = "../App_Themes/" + this.Theme + "/";
                        XmlDocument xmlDocument = new XmlDocument();
                        xmlDocument.LoadXml(playerAccessInfo.ToXml());
                        ...
                        compiledTransform.Load(this.Server.MapPath("..\\App_Themes\\" + this.Theme + "\\xsl\\PlayerAccess.xsl"));
                    ...
                    else if (playerAccessInfo.ErrorCode == CErrorCode.ErrorValidation)
