...
{
    public partial class PlayerHistory : BasePage, IRequiresSessionState
    {
            ...
            try
            {
               ...
               AddOnWebClient.Agent agentInstance = new AddOnWebClient.Agent();
                if (Common.HasRights(ReportPosition.PLAYERHISTORY))
                {
                    ...
                    if (this.Request.QueryString["cDateWeek"] != null)
                    {
                        string[] strArray = this.Request.QueryString["cDateWeek"].ToString().Split("_".ToCharArray());
                        prmStartDate = strArray.GetValue(0).ToString();
                        prmEndDate = strArray.GetValue(1).ToString();
                    }
                    ...
                    if (reportPlayerHistory.ErrorCode == CErrorCode.ErrorNone)
                    {
                        ...
                        compiledTransform.Load(this.Server.MapPath("..\\App_Themes\\" + this.Theme + "\\xsl\\PlayerHistory.xsl"));
