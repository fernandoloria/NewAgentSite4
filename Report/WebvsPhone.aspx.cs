...
{
    public partial class WebvsPhone : BasePage, IRequiresSessionState
    {
        ...
        private void LoadData()
        {
            try
            {
                ...
                AddOnWebClient.Agent agentInstance = new AddOnWebClient.Agent(); //TODO
                if (!Common.HasRights(ReportPosition.WEBVRSPHONE))
                {
                    CResultWebvsPhone cresultWebvsPhone = new CResultWebvsPhone();
                    ...
                    CResultWebvsPhone reportWebvsPhone = agentInstance.GetReportWebvsPhone(prmStartDate, prmEndDate, prmAgent, prmIdAgent, prmIdCurrency);
                    if (reportWebvsPhone.ErrorCode == CErrorCode.ErrorNone)
                    {
                        string str = "../App_Themes/" + this.Theme + "/";
                        XmlDocument xmlDocument = new XmlDocument();
                        xmlDocument.LoadXml(reportWebvsPhone.ToXml());
                        ...
                        compiledTransform.Load(this.Server.MapPath("..\\App_Themes\\" + this.Theme + "\\xsl\\WebvsPhone.xsl"));
                    ...
                    else if (reportWebvsPhone.ErrorCode == CErrorCode.ErrorValidation)
