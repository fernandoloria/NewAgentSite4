...
{
    public partial class AgentWagerListing : BasePage, IRequiresSessionState
    {
        ...
        private void LoadData()
        {
            try
            {
               ...
               AddOnWebClient.Agent agentInstance = new AddOnWebClient.Agent();
                if (Common.HasRights(ReportPosition.AGENTWAGERLISTING))
                {
                    CResultAgentWagerListing agentWagerListing1 = new CResultAgentWagerListing();
                    ...
                    CResultAgentWagerListing agentWagerListing2 = agentInstance.GetReportAgentWagerListing(prmIdAgent, prmIdPlayer, prmStartDate, prmEndDate, prmIdCurrency);
                    if (agentWagerListing2.ErrorCode == CErrorCode.ErrorNone)
                    {
                        string str = "../App_Themes/" + this.Theme + "/";
                        XmlDocument xmlDocument = new XmlDocument();
                        xmlDocument.LoadXml(agentWagerListing2.ToXml());
                        ...
                        compiledTransform.Load(this.Server.MapPath("..\\App_Themes\\" + this.Theme + "\\xsl\\AgentWagerListing.xsl"));
                    ...
                    else if (agentWagerListing2.ErrorCode == CErrorCode.ErrorValidation)
