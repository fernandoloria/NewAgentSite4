...
{
    public partial class AgentWeeklyPayments : BasePage, IRequiresSessionState
    {
        ...
        private void LoadData()
        {
            try
            {
               ...
               AddOnWebClient.Agent agentInstance = new AddOnWebClient.Agent();
                if (Common.HasRights(ReportPosition.AGENTWEEKLYPAYMENTS))
                {
                    CResultWeeklyPayments cresultWeeklyPayments = new CResultWeeklyPayments();
                    ...
                    CResultWeeklyPayments agentWeeklyPayment = agentInstance.GetReportAgentWeeklyPayment(prmIdAgent, prmDateWeek, prmIdCurrency);
                    if (agentWeeklyPayment.ErrorCode == CErrorCode.ErrorNone)
                    {
                        string str = "../App_Themes/" + this.Theme + "/";
                        XmlDocument xmlDocument = new XmlDocument();
                        xmlDocument.LoadXml(agentWeeklyPayment.ToXml());
                        ...
                        compiledTransform.Load(this.Server.MapPath("..\\App_Themes\\" + this.Theme + "\\xsl\\AgentWeeklyPayments.xsl"));
                    ...
                    else if (agentWeeklyPayment.ErrorCode == CErrorCode.ErrorValidation)
