...
{
    public partial class CashFlow : BasePage, IRequiresSessionState
    {
        ...
        private void LoadData()
        {
            try
            {
               ...
               AddOnWebClient.Agent agentInstance = new AddOnWebClient.Agent();
                if (Common.HasRights(ReportPosition.CASHFLOW))
                {
                    CResultCashFlow cresultCashFlow = new CResultCashFlow();
                    ...
                    if (this.Request.Form["Date1"] != null && this.Request.Form["Date2"] != null)
                    {
                        ...
                        num = int.Parse(this.Request.Form["Type"].ToString());
                    }
                    if (this.Request.Form["cCurrency"] != null)
                        prmIdCurrency = short.Parse(this.Request.Form["cCurrency"].ToString());
                    CResultCashFlow reportCashFlow = agentInstance.GetReportCashFlow(prmStartDate, prmEndDate, prmAgent, prmIdAgent, "", (long)num, prmIdCurrency);
                    if (reportCashFlow.ErrorCode == CErrorCode.ErrorNone)
                    {
                        string str = "../App_Themes/" + this.Theme + "/";
                        XmlDocument xmlDocument = new XmlDocument();
                        xmlDocument.LoadXml(reportCashFlow.ToXml());
                        ...
                        compiledTransform.Load(this.Server.MapPath("..\\App_Themes\\" + this.Theme + "\\xsl\\CashFlow.xsl"));
                    ...
                    else if (reportCashFlow.ErrorCode == CErrorCode.ErrorValidation)
