...
{
    public partial class HoldPercent : BasePage, IRequiresSessionState
    {
        ...
        private void LoadData()
        {
            try
            {
               ...
               AddOnWebClient.Agent agentInstance = new AddOnWebClient.Agent();
                if (Common.HasRights(ReportPosition.HOLDPERCENT))
                {
                    CResultAgentHoldPercent agentHoldPercent1 = new CResultAgentHoldPercent();
                    ...
                    CResultAgentHoldPercent agentHoldPercent2 = agentInstance.GetReportAgentHoldPercent(prmStartDate, prmEndDate, prmAgent, prmIdAgent, prmIdCurrency);
                    if (agentHoldPercent2.ErrorCode == CErrorCode.ErrorNone)
                    {
                        string str = "../App_Themes/" + this.Theme + "/";
                        XmlDocument xmlDocument = new XmlDocument();
                        xmlDocument.LoadXml(agentHoldPercent2.ToXml());
                        ...
                        compiledTransform.Load(this.Server.MapPath("..\\App_Themes\\" + this.Theme + "\\xsl\\HoldPercent.xsl"));
                    ...
                    else if (agentHoldPercent2.ErrorCode == CErrorCode.ErrorValidation)
