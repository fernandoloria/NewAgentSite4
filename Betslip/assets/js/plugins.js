...
$(document).ready(function() {
	const isSportsPage = (location.pathname || '').includes('Sports.aspx');
    if ($('#sportSide').length > 0 && isSportsPage) {
        $.get("/Wager/CreateSports.aspx", { WT: 0, auth: 'mPEV5DMcUXb4FzGG'  },
            function(result) {
                //			alert(result)
                var menu = $(result).find("#sportNav").html();
                var side = $("#sportSide").html();
                $("#sportSide").html(side);
                //			alert(menu);
    ...
    if ($('#racSide').length > 0) {
        var menu = $('#upcRac').html();
        var side = $("#racSide").html();
        $("#racSide").html(side + menu);
        //		alert(menu);
        $('.nav > li > a').click(function() {
            if ($(this).attr('class') != 'active') {
                $('.nav li ul').slideUp();
...
$(document).ready(function() {
    $(".panel-tools .closed-tool").click(function(event) {
        $(this).parents(".panel").fadeOut(400);
